...
export class StateService {
  ...
  private readonly isDropZoneInvalidSource = new BehaviorSubject<boolean>(
    false
  );
  ...
  public readonly isDropZoneInvalid$ = this.isDropZoneInvalidSource
    .asObservable()
  ...
  constructor(@Inject(DND_GRID_OPTIONS) private options: DndGridOptions) {
    ...
    return this.dropZoneSource.pipe(
      ...
      filter(
        ([prev, curr]) =>
          prev == null ||
          curr == null ||
          !BoxUtil.areBoxesEquivalent(prev, curr)
      ),
      map(([prev, curr]) => curr),
      shareReplay(1)
    );
    ...
    return this.dropZone$.pipe(
      ...
      scan((cells: Cell[], dropZone: Box) => {
        return cells.map((cell) => {
          cell.isHighlighted =
            dropZone != null && BoxUtil.isPointInBox(cell, dropZone);
          return cell;
        });
      }, cells),
      shareReplay(1)
  ...
  public widgetsExcluding(widget: Widget): Widget[] {
    return this.widgets.filter((x) => {
      return (
        x.type.id !== widget.type.id ||
        !BoxUtil.areBoxesEquivalent(x.box, widget.box)
      );
    });
  }
}
